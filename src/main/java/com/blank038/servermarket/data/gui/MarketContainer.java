...
public class MarketContainer {
	...
	public void openGui(int currentPage) {
		...
		if (data.contains("items")) {
			for (String key : data.getConfigurationSection("items").getKeys(false)) {
				...
				if (section.contains("action")) {
					itemStack = ServerMarket.getInstance().getNBTBase().addTag(itemStack, "MarketAction",
		...
		// 开始获取全球市场物品
		Integer[] slots = ServerMarket.getInstance().getApi().getInt(data.getStringList("sale-item-slots"))
		...
		List<String> extrasLore1 = data.getStringList("sale-info-owner");
		...
		for (int i = start, index = 0; i < end; i++, index++) {
			...
			//////////////////// MrXiaoM 修改 ////////////////////
			// 让卖家和买家显示不一样的信息
			items.put(slots[index], getShowItem(saleItem, this.player.getName().equals(saleItem.getOwnerName())?extrasLore1:extrasLore));
		}
		...
		guiModel.execute((e) -> {
			...
			if (e.getClickedInventory() == e.getInventory()) {
				...
				if (key != null) {
					...
					// 判断交互方式
					//////////////////// MrXiaoM 修改 ////////////////////
					// 改switch为if else，并增加命令交互方式
					if(action.equalsIgnoreCase("up")) {
						if (lastPage == 1) {
							clicker.sendMessage(I18n.trans("no-previous-page", true));
						} else {
							openGui(lastPage - 1);
						}
					}
						...
						if (lastPage >= finalMaxPage) {
							clicker.sendMessage(I18n.trans("no-next-page", true));
						} else {
							openGui(lastPage + 1);
						}
					}
						...
						String cmd = action.substring(7).replace('&', ChatColor.COLOR_CHAR).replace("%player%", clicker.getName());
						Bukkit.dispatchCommand(clicker, cmd);
					}
					else if(action.toLowerCase().startsWith("console:")) {
						String cmd = action.substring(8).replace('&', ChatColor.COLOR_CHAR).replace("%player%", clicker.getName());
	...
	public void buySaleItem(Player buyer, String uuid, boolean shift) {
		...
		if (!ServerMarket.getInstance().sales.containsKey(uuid)) {
			buyer.sendMessage(I18n.trans("error-sale", true));
			//////////////////// MrXiaoM 增加 START ////////////////////
			// 打开新的市场菜单以刷新GUI
			this.openGui(page);
			//////////////////// MrXiaoM 增加 END ////////////////////
		...
		if (saleItem.getOwnerName().equals(buyer.getName())) {
			if (shift) {
				...
				buyer.sendMessage(I18n.trans("unsale", true));
				this.openGui(page);
			} else {
				buyer.sendMessage(I18n.trans("shift-unsale", true));
			}
			return;
		}
		if (shift) {
			if (ServerMarket.getInstance().getEconomyBridge().balance(buyer) < saleItem.getPrice()) {
				buyer.sendMessage(I18n.trans("lack-money", true));
				return;
			}
			...
			if (seller != null && seller.isOnline()) {
				...
				seller.sendMessage(I18n.trans("sale-sell", true)
			...
			buyer.sendMessage(I18n.trans("shift-buy", true));
		}
	}
	...
	public ItemStack getShowItem(SaleItem saleItem, List<String> infoLore) {
		...
		if (ServerMarket.getInstance().getConfig().getBoolean("sale-info")) {
			...
			SimpleDateFormat sdf = new SimpleDateFormat(
					ServerMarket.getInstance().getConfig().getString("simple-date-format"));
			// 设置额外信息
