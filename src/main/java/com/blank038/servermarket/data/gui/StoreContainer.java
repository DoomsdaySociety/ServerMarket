...
public class StoreContainer {
	...
	public void open(int currentPage) {
		...
		if (data.contains("items")) {
			for (String key : data.getConfigurationSection("items").getKeys(false)) {
				...
				if (section.contains("action")) {
					itemStack = ServerMarket.getInstance().getNBTBase().addTag(itemStack, "action",
		...
		Integer[] slots = ServerMarket.getInstance().getApi().getInt(data.getStringList("store-item-slots"))
...
//        maxPage += storeItems.size() % slots.length == 0 ? 0 : 1;
		...
		for (int i = start, index = 0; i < end; i++, index++) {
			if (index >= slots.length || i >= keys.length)
				break;
			items.put(slots[index],
					ServerMarket.getInstance().getNBTBase().addTag(storeItems.get(keys[i]), "StoreID", keys[i]));
		}
		// 界面物品设置结束
		...
		guiModel.execute((e) -> {
			...
			if (e.getClickedInventory() == e.getInventory()) {
				...
				if (action != null) {
					if(action.equalsIgnoreCase("market")) {
	...
	public void getItem(Player player, String uuid) {
		...
		if (data.contains(uuid)) {
			...
			player.sendMessage(I18n.trans("get-store-item", true).replace("%item%", displayMmae)
					.replace("%amount%", String.valueOf(itemStack.getAmount())));
			// 刷新玩家界面
			new StoreContainer(player, marketPage).open(1);
		} else {
			player.sendMessage(I18n.trans("error-store", true));
		}
	}
}
